...
PROPER\(text\)将文本字符串的首字母转换成大写，将其余字母转换为小写

#### Round函数

1. 作用：将数字四舍五入到指定的位数
2. 语法：ROUND\(number, num\_digits\)，number要四舍五入的数字，num\_digits需要四舍五入运算的位数，＞0四舍五入到指定的小数位数，＝0四舍五入到最接近的整数，＜0四舍五入到小数点左边相应位数。
3. RANDUP函数语法同RAND，只是采用的使用将数字向上舍入而非四舍五入
4. RANDDOWN语法同RAND，只是采用的是将数字向下舍去而非四舍五入
