...
* 用大样本数据预测小样本数据；可以看第一个数列，分别预测第二个数和第三个数，应该是预测第三个数的误差更大吧，因此提供的信息越多，所预测的内容越少，才能更加精准地进行预测；
* 提供更多的样本量；前面所一直在说所提到的信息到底是什么，我认为和样本量的数量有关，样本量越多，所包含的信息也就越多；在一个教育测量任务中，样本量也就等同于被测学生的数量，可能对于我来说，更主要的是被测学生的数量，这些也是可以进行把控的，如果选择限制被测学生的数量，而增添题目的数量，是否还要考虑测试时间过长，引起学生反感的因素，这些因素实际上是很难进行度量分析；
